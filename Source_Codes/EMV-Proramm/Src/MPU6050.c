...
double lageAbfragen(uint8_t address) // buf ist bereits mit Lage beschrieben delay ist noch fraglich
{
...
void foehnlageberechnung(double *rotation) // Buffer leange von 5, um auch x,y,z wegen debugging abzufragen
{
	...
	if(counter == 0)
	{
			x = lageAbfragen(ACCEL_XOUT); // Wert[0] im Array Rotation
			rotation[0] = x;
	}
	else if(counter == 1)
	{
			y = lageAbfragen(ACCEL_YOUT); //Wert [1] im Array Rotation
			rotation[1] = y;
	}
	else if(counter == 2)
	{
			z = lageAbfragen(ACCEL_ZOUT);	//Wert[2] im Array Rotation
			rotation[2] = z;
	}
